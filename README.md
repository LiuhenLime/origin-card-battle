...
- **资源**：部署费用可累计——防守方每回合获得 20；进攻方按回合递增：第 1-5 回合每回合 20、第 6-10 回合开始时每回合 30、第 11-15 回合开始时每回合 40。80 张道具牌入公共牌库，首回合各抽 5 张、每回合结束各抽 2 张。
- **行动**：防守方先手、双方轮流；每次行动可不限次用道具牌，再上阵 / 下阵 / 使用技能 / 结束回合。**从第 8 回合起，进攻方一次行动可连续部署多位角色**（上阵不再消耗行动权）。
