...
- **阵营**：单机模式，玩家固定为防守方；守住总生命 10 撑过 10 回合获胜，被 AI 进攻方扣到 0 则失败。
- **资源**：部署费用可累计——防守方每回合获得 20；进攻方按回合递增：第 1-5 回合每回合 20、第 6-7 回合开始时 30、第 8 回合开始时 100、第 9 回合开始时 150、第 10 回合开始时 200。80 张道具牌入公共牌库，首回合各抽 5 张、每回合结束各抽 2 张。
- **行动**：防守方先手、双方轮流；每次行动可不限次用道具牌，再上阵 / 下阵 / 使用技能 / 结束回合。**第 8 回合开始时进攻方死亡冷却全部清零，且从第 8 回合起一次行动可连续部署多位角色**（上阵不再消耗行动权）。
- **站位**：防守方 2×3 共 6 个区域、进攻方 3×3 共 9 个区域，每区限放一个角色；防守方 2 个高地（后排两角）+ 4 个地面，地面上高地费用翻倍；进攻方无高地、永不翻倍。
...
- **角色**：费用 / 生命 / 攻击 / 技能点 / 阵营攻防值 + 被动；普通攻击不耗点，大招满点清空，死亡冷却 4 回合（第 8 回合开始时进攻方冷却清零）后费用递增。
